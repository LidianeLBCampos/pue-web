	...
	<div class="container jumbotron">
		...
		<select onchange="refrescar('filterBooks.do', this.value)"
			...
			<c:forEach var="cat" items="${todas}">
				<c:choose>
					<c:when test="${(!empty filtrados) && (cat.id eq catRecibida)}">
			...
			<tbody>
				...
				<c:forEach var="libro" items="${libros}">
					<tr>
						<td>${libro.isbn}</td>
						<td>${libro.titulo}</td>
						<td><a style="color:black" href="#" onclick="refrescar('filterBooks.do','${libro.categoria.id}')">
									${libro.categoria.descripcion}</a></td>
						<td align="center"><a
